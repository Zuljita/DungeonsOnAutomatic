...
class SVGExportPlugin implements ExportPlugin {
  ...
  private generateMetadataComment(d: Dungeon, opts: SVGRenderOptions): string {
    const timestamp = new Date().toISOString();
    const roomCount = d.rooms?.length || 0;
    const doorCount = d.doors?.length || 0;
    const corridorCount = d.corridors?.length || 0;
    
    return `<!-- Generated by DungeonsOnAutomatic (DOA) -->
<!-- Generated: ${timestamp} -->
<!-- Seed: ${d.seed || 'none'} -->
<!-- Rooms: ${roomCount} -->
<!-- Doors: ${doorCount} -->
...
<!-- Theme: ${opts.theme || 'light'} -->`;
  ...
  private async renderSvg(d: Dungeon, opts: SVGRenderOptions): Promise<string> {
    ...
    // Calculate bounds including all elements (rooms, corridors, doors, text)
    let allXCoords: number[] = [];
    let allYCoords: number[] = [];
    
    // Collect room bounds
    for (const r of d.rooms) {
      const roomPixelX = r.x * cellSize;
      const roomPixelY = r.y * cellSize;
      const roomPixelW = r.w * cellSize;
      const roomPixelH = r.h * cellSize;
      
      allXCoords.push(roomPixelX, roomPixelX + roomPixelW);
      allYCoords.push(roomPixelY, roomPixelY + roomPixelH);
    }
    
    // Collect corridor bounds
    for (const c of d.corridors) {
      for (const p of c.path) {
        const pixelX = p.x * cellSize;
        const pixelY = p.y * cellSize;
        allXCoords.push(pixelX, pixelX + cellSize);
        allYCoords.push(pixelY, pixelY + cellSize);
      }
    }
    
    // Collect door positions (they can extend beyond room boundaries)
    for (const door of d.doors || []) {
      if (door.location) {
        const doorPixelX = door.location.x * cellSize;
        const doorPixelY = door.location.y * cellSize;
        // Doors can have visual elements extending beyond their grid cell
        allXCoords.push(doorPixelX - cellSize, doorPixelX + cellSize * 2);
        allYCoords.push(doorPixelY - cellSize, doorPixelY + cellSize * 2);
      }
    }
    
    // Calculate content bounds
    const minX = Math.min(...allXCoords, 0);
    const minY = Math.min(...allYCoords, 0);
    const maxX = Math.max(...allXCoords, cellSize);
    const maxY = Math.max(...allYCoords, cellSize);
    ...
    const parts: string[] = [
      ...
      this.generateMetadataComment(d, opts),
    ];
  ...
  private renderHexSvg(d: Dungeon, theme: RenderTheme, opts: { hexSize: number; showGrid: boolean }): string {
    ...
    const parts: string[] = [
      `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">`,
      this.generateMetadataComment(d, { style: 'hex', theme: 'light' }),
  ...
  private renderGridlessSvg(d: Dungeon, theme: RenderTheme, cellSize: number): string {
    // Calculate map bounds in grid coordinates then convert to pixels
    const points: { x: number; y: number }[] = [];
    for (const r of d.rooms) {
      if (r.shape === 'rectangular' || !r.shapePoints) {
        // Include both top-left and bottom-right corners for proper bounds
        points.push({ x: r.x, y: r.y });
        points.push({ x: r.x + r.w, y: r.y + r.h });
      } else {
        const bounds = roomShapeService.getRoomBounds(r);
        points.push({ x: Math.floor(bounds.minX), y: Math.floor(bounds.minY) });
    ...
    const svgBounds = calculateGridBounds(points, 2); // Add more padding for visual clarity
    const width = svgBounds.width * cellSize;
    const height = svgBounds.height * cellSize;
    
    const parts: string[] = [
      `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">`,
      this.generateMetadataComment(d, { style: 'gridless', theme: 'light' }),
      `<rect x="0" y="0" width="${width}" height="${height}" fill="${theme.background}"/>`,
    ];
